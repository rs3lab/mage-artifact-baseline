...
constexpr static uint64_t kSampleGap = 8ULL; /* Sample every 16 access */
                            ...
                            if (write_test == true){
                                ...
                                *(test_array + i) = 2;
                                if (latency_test && (cnt % kSampleGap == 0)){
                                    ...
                                    latency_sample[per_thread_size / kPageSize * tid / kSampleGap + cnt / kSampleGap] = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
                                }
                            }else {
                                if (latency_test && (cnt % kSampleGap == 0)){
                                    start = std::chrono::high_resolution_clock::now();
